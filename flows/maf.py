...
class MADE(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        mu, alpha = self.net(self.net_input(x, y)).chunk(chunks=2, dim=1)
    ...
    def inverse(self, u, y=None, sum_log_abs_det_jacobians=None):
        ...
        for i in self.input_degrees:
            mu, alpha = self.net(self.net_input(x.clone(), y)).chunk(chunks=2, dim=1)
            x[:,i] = mu[:,i] + u[:,i] * torch.exp(alpha[:,i])
        logdet = alpha
        return x, logdet

    def log_prob(self, x, y=None):
        u, logdet = self.forward(x, y)
        return torch.sum(self.base_dist.log_prob(u) + logdet, dim=1, keepdim=True)
...
class MAF(nn.Module):
    ...
    def inverse(self, u, y=None):
        gen, logdet =  self.net.inverse(u, y)
        
        # get logq(x̃), where x̃ = f^{-1}(z)
        logq_gen = torch.sum(self.base_dist.log_prob(u) - logdet, dim=1, keepdim=True)
